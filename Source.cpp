...
int main()
{
		...
		for (int j = i + 1; j < number[i].size(); j++)
		{
			if (j < number[i].size())
			{
				if (number[i] == number[j])
				{
					...
					//QQQ.letterterm.erase(QQQ.letterterm.begin() + j);
					//QQQ.letterterm.erase(QQQ.letterterm.begin() + i);
				}
			}
		}
	}
	//for (int i = 0; i <QQQ.letterterm.size();i++)
	//{
	//	final.push_back(QQQ.letterterm[i]);
	//}
	vector<vector<int>> finalnum(final.size()+QQQ.remainterm.size());
	...
	vector<string>x(minterms.size());
	for (int i = 0; i < x.size(); i++)
	{
		x[i].resize(endgame.size());
	}
	for (int i = 0; i < endgame.size(); i++)
	{
		cout << setw(6) << endgame[i] << ",";
	}
	int count = final.size();

	for (int i = 0; i < QQQ.remainterm.size(); i++) {
		finalnum[count].push_back(atoi(QQQ.remainterm[i].data()));
		count++;
	...
	for (int i = 0; i < finalnum.size(); i++)
	{
		for (int j = 0; j < finalnum[i].size(); j++)
		{
			for (int thanos = 0; thanos < endgame.size(); thanos++)

			{
				if (finalnum[i][j] == endgame[thanos])
				{
					x[i][thanos] = 'x';
	...
	for (int i = 0; i < minterms.size(); i++)
	{
		cout << setw(20) << left << QQQ.getValue(minterms[i]) << "|      ";
		for (int j = 0; j < x[i].size(); j++)
		{
			(x[i][j] == 'x') ? cout << x[i][j]<<"      " : cout << "       ";
		}
		cout << endl;
	}
	cout << "--------------------+-------------------------------------------------------------------------------" << endl;
